...
public class PingNative {
    ...
    public static PingResult ping(InetAddress host, PingOptions pingOptions) throws IOException, InterruptedException {
        ...
        if (host == null) {
            pingResult.isReachable = false; pingResult.error = "Address ie empty";
            return pingResult;
        }
        ...
        if (address != null) {
            if (IPUtils.isIPv6Address(address)) {
                // If we detect this is a ipv6 address, change the to the ping6 binary
                pingCommand = "ping6";
            } else if (!IPUtils.isIPv4Address(address)) {
    ...
    public static PingResult getPingStats(PingResult pingResult, String s) {
        ...
        if (s.contains("0% packet loss")) {
            ...
            if (start == -1 || end == -1) {
                ...
                pingResult.result = s;
                pingResult.timeTaken = Float.parseFloat(stats[1]); //getting avg time from stats
                return pingResult;
            }
